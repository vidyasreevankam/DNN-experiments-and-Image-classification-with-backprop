...
# Load Data
print("Loading Data...")
train = pd.read_csv(args.train)#give train.csv path in place of args.train
test = pd.read_csv(args.test)#give test.csv path in place of args.test
val = pd.read_csv(args.val)#give val.csv path in place of val.test
train_x = np.array(train.drop(columns=["id", "label"], axis=1))#drop id,label columns first and last column
...
file_writer = open(args.save_dir + "losses.txt", "a")
